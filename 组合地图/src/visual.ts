...
export default class Visual extends WynVisual {
  ...
  private timerId1 = null;
  private timerId2 = null;
  private timerId3 = null;
  ...
  private render() {
    ...
    let renderEachArea = () => {
      ...
      echarts.util.each(areaData, function(dataItem: any, idx) {
    ...
    let throttleRender = (fn, delay, debounce) => {
      let currCall, lastCall = 0, lastExec = 0, diff, scope, args;
      delay = delay || 0;
      function exec() {
      ...
      let callRender = function() {
        ...
        if (debounce) {
          this.timerId1 = setTimeout(exec, delay);
        } else {
          if (diff >= 0) {
            exec();
          } else {
            this.timerId1 = setTimeout(exec, -diff);
          }
        }
        lastCall = currCall;
      };
