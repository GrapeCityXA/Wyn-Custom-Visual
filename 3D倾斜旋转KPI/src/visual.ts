...
export default class Visual extends WynVisual {
  ...
  public update(updateOptions: VisualNS.IVisualUpdateOptions) {
    ...
    if (this.isMock) {
      ...
      this.value = MockDataAttr.Actual;
      this.contrast = MockDataAttr.Contrast;
  ...
  public render() {
    ...
    for (var i = 0; i < rotateY.length; i++) {
      ...
      if (this.isMock || this.isRate) {
        ...
        const rateText = $('<span></span>').text(MockDataAttr.Rate);
  ...
  public export() {
    ...
    firstItem[MockDataAttr.Rate] = rate;
    let text = '';
    ...
    html2canvas(this.dom, { backgroundColor: null }).then((canvas) => {
