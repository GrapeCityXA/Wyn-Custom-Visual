...
export default class Visual extends WynVisual {
  ...
  private selectEvent(data,sid,properties) {
    ...
    switch (leftMouseButton) {
      //鼠标联动设置    
      case "none": {
        this.doSelection(properties, sid);
        break;
      }
      case "Jump": {
        this.doSelection(properties, sid);
  ...
  private doSelection(properties, sid) {
  ...
  public export() {
    ...
    this.dom.style.removeProperty('transform-origin');
    const def = this.host.promiseFactory.defer();
    const span = document.createElement('span');
    span.style.display = 'inline-block';
    span.style.width = `${this.viewport.width}px`;
    span.style.height = `${this.viewport.height}px`;
    let text = '';
    ...
    html2canvas(span, { backgroundColor: null }).then((canvas) => {
