...
export default class Visual extends WynVisual {
  ...
  private finishedChartCount: number;
  private echartsArray = [];
  ...
  private renderProgress() {
    ...
    for (let i = 0; i < this.progressBoxes.length; i++) {
      ...
      this.echartsArray.push(progressInstance);
  ...
  private renderChart() {
    this.finishedChartCount = 0;
    for (let i = 0; i < this.chartBoxes.length; i++) {
      const eChartsInstance =  this.host.moduleManager.getModule('echarts').init(this.chartBoxes[i]);
      this.echartsArray.push(eChartsInstance);
      ...
      eChartsInstance.on('finished', () => {
        this.finishedChartCount++;
        if (this.finishedChartCount === this.chartBoxes.length) {
  ...
  private doSelection(selectionId) {
  ...
  public onDestroy() {
    this.echartsArray.forEach((chart)=>{
      chart.dispose();
    });
  ...
  public async export() {
    ...
    const canvas = await html2canvas(this.dom, {windowWidth: rect.width, windowHeight: rect.height, backgroundColor: null});
