...
export default class Visual extends WynVisual{
  ...
  private buildFields = (params) => {
    ...
    const actualTip = items[1][params.dataIndex]
    const contrast = items[2][params.dataIndex]
    this.isDimension && fields.push({ label: this.Dimension, value: items[0][params.dataIndex] })
    this.isActualValue && fields.push({ label: this.ActualValue, value: this.formatData(actualTip, options.showSecondBarActualUnit, this.actualFormate) });
    this.isContrastValue && fields.push({ label: this.ContrastValue, value: this.formatData(contrast, options.showSecondBarContrastUnit, this.contrastFormate) })
    if (this.items.length > 7) {
      this.items[7].forEach((val, index) => {
        fields.push(
          { label: val, value: this.items[8 + index][dataIndex] }
        );
      });
    }
    return fields;
  ...
  private clickFunction = (params: any) => {
    if (!params.seriesClick) {
      this.hideTooltip();
  ...
  public bindEvents = () => {
    ...
    this.chart.on('mousemove', (params) => {
      ...
      if (!this.isTooltipModelShown) {
        ...
        this.host.toolTipService.show({
          ...
          fields: this.buildFields(params),
        });
      }
    });
    this.chart.on('click', (params) => {
      ...
      if (clickMouse === clickLeftMouse) {
        ...
        // show data jump
        switch (this.properties.clickLeftMouse) {
          case 'none':
            return;
          case 'showToolTip':
            ...
            break;
          case 'Jump':
            const sid = this.items[params.dataIndex].selectionId;
  ...
  private render() {
    ...
    let option = {
      dataZoom: null,
      animationDuration: 0,
      animationDurationUpdate: this.properties.animationDurationUpdate * 1000,
      animationEasing: this.properties.animationStyle,
      animationEasingUpdate: this.properties.animationStyle,
      grid: {
        top: '10',
      ...
      option.dataZoom = [
        {
          ...
          startValue: 0, // 从头开始。
          endValue: this.properties.animationShowNumber - 1
        }
      ],
        this.beginAnimation(option);
    } else {
  ...
  public beginAnimation(option) {
    if (this.setIntervalId != null) {
      clearInterval(this.setIntervalId);
      this.setIntervalId = null;
    }
    if (this.properties.animationDurationUpdate > this.properties.animationTime) {
      this.host.propertyService.setProperty('animationDurationUpdate', this.properties.animationTime);
    }
    if (this.properties.animationShowNumber > this.items[0].length) {
      this.host.propertyService.setProperty('animationShowNumber', this.items[0].length);
    }
    let index = this.items[0].length
    let mychart = this.chart;
    let animationShowNumber = this.properties.animationShowNumber
    let that = this;
    function run() {
      // 每次向后滚动一个，最后一个从头开始。

      let currentStartNum = option.dataZoom[0].startValue;
      let currentEndNum = option.dataZoom[0].endValue;

      if (currentStartNum == (index - animationShowNumber)) {
        //当轮播到最后一条数据时，重新开始 start 
        option.dataZoom[0].startValue = 0;
        mychart.resize();
        that.render();
        //当轮播到最后一条数据时，重新开始 end
      } else {
        //解决数值不跟随轮播问题 start
        let tempArr = [];
        for (let index = 0; index < that.items[3].length; index++) {
          const item = that.items[3][index];
          if (index > 0) {
            tempArr.push(item);
          }
        }
        tempArr.push(that.items[3][0]);
        that.items[3] = tempArr;
        //解决数值不跟随轮播问题 end

        option.dataZoom[0].endValue = currentEndNum + 1;
        option.dataZoom[0].startValue = currentStartNum + 1;
        mychart.setOption(option);
      }
    }
    this.setIntervalId = setInterval(function () {
      run();
    }, this.properties.animationTime * 1000);
  }
  ...
  public onDestroy(): void {
    ...
    this.container.removeEventListener('contextmenu', this.showContextMenu);
  }
  ...
  public getInspectorHiddenState(updateOptions: any): string[] {
    ...
    if (updateOptions.dataViews.length > 0) {
      ...
      if (!updateOptions.properties.animation) {
        hiddenOptions = hiddenOptions.concat(['animationStyle', 'animationShowNumber', 'animationTime', 'animationDurationUpdate'])
      }
